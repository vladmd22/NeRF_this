...
def main():
    ...
    for i in trange(start_iter, cfg.experiment.train_iters):
        ...
        if USE_CACHED_DATASET:
            ...
            target_ray_values_full = cache_dict["target"][..., :3].reshape((-1, 3))
            select_inds = np.random.choice(
                ray_origins_full.shape[0],
                size=(cfg.nerf.train.num_random_rays),
                replace=False,
            )
            ray_origins, ray_directions = (
                ray_origins_full[select_inds],
                ray_directions_full[select_inds],
            )
            target_ray_values = target_ray_values_full[select_inds].to(device)
